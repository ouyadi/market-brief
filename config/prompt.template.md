...
- **微信** 每个群 `mcp__chatlog__wx_history(chat=<chatroom_id>, since=since, limit=100)`
...
- **微信** `wx_history(chat=..., since=since, until=<最老 ts>, limit=100)`
...
`python ~/Scripts/market-brief/discord_image_ocr.py --channel-id <CHANNEL_ID> --since-hours 18 --limit 80 --max-images 15 --backend auto`

worker 会输出 `MARKDOWN_WRITTEN` / `JSONL_WRITTEN`;Read markdown 后按以下规则处理:
- `ocr_text` 非空 → 把 OCR 文本当作该 KOL/频道的实质发言处理
- `status: needs_vision` → 用同条 `attachment_url` 继续 `WebFetch(url)` 让 vision 读图,但不要把 signed URL 写入最终 report
