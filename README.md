...
第二个 HTTP MCP server(`stock_price_mcp.py`,端口 3032),基于 `yfinance`。免费、无 auth。**9 个工具,分两组**:
...
第四个 HTTP MCP server(`polymarket_mcp.py`,端口 3033),基于 Polymarket 的 Gamma API。**预测市场价格 = 市场隐含概率**,money-weighted、实时,比新闻头条更早动。免费、无 auth。5 个工具:

- **`list_markets(query, limit, active_only, sort_by)`** — 市场列表,query 模糊匹配 question,sort_by 可选 volume24hr/volume1wk/volume1mo/liquidity/endDate
- **`get_market(id_or_slug)`** — 单个市场完整字段
- **`list_events(query, limit, active_only)`** — 事件列表(一个事件聚合多个相关市场,比如 "Fed 6 月会议" 下挂 25bp/50bp/hold 三选项)
- **`get_event(id_or_slug)`** — 单个事件 + 全部子市场
- **`top_movers(window='1mo', limit)`** — 月度概率变动最大的市场(Gamma 没暴露 1d/1w 价变,这是唯一可用窗口)

简报用法:Fed 决议 / CPI 数据 / 大选辩论 / 地缘事件 / 加密里程碑 都是 Polymarket 重点市场。**跟群里 / KOL 的 call 交叉验证特别有用**:群里说"肯定降 50bp"vs Polymarket 实际定价 30% → 信号偏差量化。

跑在 `127.0.0.1:3033/mcp`,scheduled task `PolymarketMCP` / launchd `com.ouyadi.polymarket-mcp`。**只读**,纯 HTTP GET,Polygon 链上的下单部分(CLOB API)我们不暴露。
...
5. **注册 5 个 scheduled task**(MarketBrief / WeixinListener / TwitterMCP / StockPriceMCP / PolymarketMCP)—— 全部通过 `wscript.exe` + `run-hidden.vbs` 启动,**触发时不再闪 PowerShell 窗口**
...
| `PolymarketMCP` *(可选)* | Polymarket Gamma 守护(127.0.0.1:3033) |
...
| &nbsp;&nbsp;[`install-twitter-mcp.ps1`](scripts/windows/install-twitter-mcp.ps1) / [`install-stock-mcp.ps1`](scripts/windows/install-stock-mcp.ps1) / [`install-polymarket-mcp.ps1`](scripts/windows/install-polymarket-mcp.ps1) | MCP 守护任务注册 |
...
| &nbsp;&nbsp;[`polymarket_mcp.py`](mcp/polymarket_mcp.py) | HTTP MCP:Polymarket Gamma → list_markets/get_market/list_events/get_event/top_movers |
